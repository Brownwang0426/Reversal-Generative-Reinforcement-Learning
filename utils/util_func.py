...
def update_future_action(itrtn_for_planning,
    ...
    for _ in range(itrtn_for_planning):
        ...
        envisaged_reward, \
        envisaged_state    = model.forward(history_state, history_action, present_state, future_action_)
...
def update_model(itrtn_for_learning,
    ...
    for _ in tqdm(range(itrtn_for_learning)):
        ...
        model.train()
        model.unlock()
        _, _ = model.forward_(history_state, history_action, present_state, future_state, future_action)
        model.lock()
