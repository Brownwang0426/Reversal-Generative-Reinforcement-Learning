...
def update_future_action(itrtn_for_planning,
    ...
    for _ in range(itrtn_for_planning):
        ...
        future_action_ = future_action.detach().clone().requires_grad_(True)
        softmax_action = (torch.softmax(future_action_, dim=-1) - 0.5) * 2
        ...
        envisaged_reward, envisaged_state = model(history_state, history_action, present_state, softmax_action)
        total_loss = loss_function(envisaged_reward, desired_reward)
        total_loss.backward()

        future_action = future_action - beta * future_action_.grad
