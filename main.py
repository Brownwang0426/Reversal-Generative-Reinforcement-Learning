...
from utils.util_func  import make_buffer,\
...
if load_pretrained_model == True:
    try:
        ...
        buffer = load_buffer_from_pickle(buffer_directory)
        buffer.to(device_)
...
for training_episode in tqdm(range(episode_for_training)):
    ...
    while not done_truncated_flag:
        ...
        history_state   = retrieve_history(state_list, history_size, device_)
    ...
    buffer.add_episode(current_episode, history_state_list, present_state_list, future_action_list, future_reward_list)
    ...
    if current_episode % episode_for_validation == 0:
        ...
        save_buffer_to_pickle(buffer_directory, buffer)
