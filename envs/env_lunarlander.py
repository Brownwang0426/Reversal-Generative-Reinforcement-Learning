...
def vectorizing_state(state):  # Reminder: change this for your specific task ⚠️⚠️⚠️
    state_0 = quantifying(-1, 1, 100, -1.5 , 1.5  , state[0]) 
    state_1 = quantifying(-1, 1, 100, -1.5 , 1.5  , state[1]) 
    state_2 = quantifying(-1, 1, 100, -5   , 5    , state[2]) 
    state_3 = quantifying(-1, 1, 100, -5   , 5    , state[3]) 
    state_4 = quantifying(-1, 1, 100, -3.14, 3.14 , state[4])   
    state_5 = quantifying(-1, 1, 100, -5   , 5    , state[5])   
    state_6 = quantifying(-1, 1, 100, 0    , 1    , state[6])    
    state_7 = quantifying(-1, 1, 100, 0    , 1    , state[7])    
...
def vectorizing_reward(state, reward, summed_reward, done, reward_size):       # Reminder: change this for your specific task ⚠️⚠️⚠️
    reward = quantifying(0, 1, reward_size, -200, 325, reward)       
    return reward
