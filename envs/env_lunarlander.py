...
def vectorizing_state(state, done, truncated, device):  # Reminder: change this for your specific task ⚠️⚠️⚠️
    ...
    state_1 = quantifying(-1, 1, 25, -2.5 , 2.5  , state[0], device) 
    state_2 = quantifying(-1, 1, 25, -2.5 , 2.5  , state[1], device) 
    state_3 = quantifying(-1, 1, 25, -10  , 10   , state[2], device) 
    state_4 = quantifying(-1, 1, 25, -10  , 10   , state[3], device) 
    state_5 = quantifying(-1, 1, 25, -6.28, 6.28 , state[4], device)   
    state_6 = quantifying(-1, 1, 25, -10  , 10   , state[5], device)   
    state_7 = quantifying(-1, 1, 25, 0    , 1    , state[6], device)    
    state_8 = quantifying(-1, 1, 25, 0    , 1    , state[7], device)    
...
def vectorizing_reward(state, done, truncated, reward, summed_reward, reward_size, device):       # Reminder: change this for your specific task ⚠️⚠️⚠️
    if done or truncated: 
        if done:
            reward = quantifying(-1, 1, reward_size , -200, 350, summed_reward, device)       
        else:
            reward = quantifying(-1, 1, reward_size , -200, 350, summed_reward, device)   
    else:
        reward = quantifying(-1, 1, reward_size , -200, 350, summed_reward, device)       
