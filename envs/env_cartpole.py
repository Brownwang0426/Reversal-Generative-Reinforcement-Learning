...
def vectorizing_state(state, done, truncated, device, time_steps=0):      # Reminder: change this for your specific task ⚠️⚠️⚠️
    ...
    state_1 = quantifying(-1, 1, 100, -4.8  , 4.8   , state[0], device)
    state_2 = quantifying(-1, 1, 100, -3.75 , 3.75  , state[1], device)
    state_3 = quantifying(-1, 1, 100, -0.418, 0.418 , state[2], device)
    state_4 = quantifying(-1, 1, 100, -3.75 , 3.75  , state[3], device)
    state_t = quantifying(-1, 1, 100, 0     , 1000  , time_steps, device)
