...
def vectorizing_state(state, done, truncated, device, time_steps=0):      # Reminder: change this for your specific task ⚠️⚠️⚠️
    ...
    state_1 = quantifying(0, 1, 100, -4.8  , 4.8   , state[0], device)
    state_2 = quantifying(0, 1, 100, -3.75 , 3.75  , state[1], device)
    state_3 = quantifying(0, 1, 100, -0.418, 0.418 , state[2], device)
    state_4 = quantifying(0, 1, 100, -3.75 , 3.75  , state[3], device)
    state_t = quantifying(0, 1, 100, 0     , 1000  , time_steps, device)
...
def vectorizing_reward(state, done, truncated, reward, summed_reward, reward_size, device):       # Reminder: change this for your specific task ⚠️⚠️⚠️
    reward = quantifying(0, 1, reward_size, 0, 1000, summed_reward, device)
    return reward
