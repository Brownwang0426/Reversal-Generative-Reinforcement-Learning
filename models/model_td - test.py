...
class RMSNorm(nn.Module):
    ...
    def forward(self, x):
        norm_x = x / (x.pow(2).mean(-1, keepdim=True).add(self.eps).sqrt())
...
class build_model(nn.Module):
    def __init__(self,
        ...
        self.state_linear_res     = nn.Linear(self.state_size  , self.feature_size, bias=self.bias)
        self.action_linear_res    = nn.Linear(self.action_size , self.feature_size, bias=self.bias)
        ...
        self.transformer_norm     = RMSNorm(self.feature_size) 
        ...
        self.state_linear_out     = nn.Linear(self.feature_size, self.state_size   , bias=self.bias)
        self.state_linear_out_res = nn.Linear(self.feature_size, self.state_size   , bias=self.bias)

        self.state_norm           = RMSNorm(self.feature_size)
        self.action_norm          = RMSNorm(self.feature_size)
    ...
    def forward(self, history_s, history_a, present_s, future_s, future_a):
        ...
        if history_s.size(1) > 0:
            history_s = self.state_norm (self.state_linear (history_s) ) + self.state_linear_res (history_s)
            history_a = self.action_norm(self.action_linear(history_a) ) + self.action_linear_res(history_a)
        present_s = self.state_norm (self.state_linear (present_s.unsqueeze(1))) + self.state_linear_res (present_s.unsqueeze(1))
        future_a  = self.action_norm(self.action_linear(future_a              )) + self.action_linear_res(future_a              )
        ...
        for i in range(future_a.size(1)):
            ...
            present_s = s
            present_s = self.state_norm(self.state_linear(present_s)) + self.state_linear_res(present_s)
