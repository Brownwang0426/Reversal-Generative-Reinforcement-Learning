...
class custom_attn(nn.Module):
    def __init__(self, feature_size, num_heads, bias, drop_rate):
        ...
        self.head_size     = self.feature_size
        self.bias          = bias
        self.drop_rate     = drop_rate
        self.W_q           = nn.Linear(self.feature_size                 , self.feature_size * self.num_heads, bias=self.bias)
        self.W_q_          = nn.Linear(self.feature_size * self.num_heads, self.feature_size * self.num_heads, bias=self.bias)
        self.W_k           = nn.Linear(self.feature_size                 , self.feature_size * self.num_heads, bias=self.bias)
        self.W_k_          = nn.Linear(self.feature_size * self.num_heads, self.feature_size * self.num_heads, bias=self.bias)
        self.W_v           = nn.Linear(self.feature_size                 , self.feature_size * self.num_heads, bias=self.bias)
        self.W_v_          = nn.Linear(self.feature_size * self.num_heads, self.feature_size * self.num_heads, bias=self.bias)
        self.W_o           = nn.Linear(self.feature_size * self.num_heads, self.feature_size * self.num_heads, bias=self.bias)
        self.W_o_          = nn.Linear(self.feature_size * self.num_heads, self.feature_size                 , bias=self.bias)
    ...
    def combine_heads(self, x):
        ...
        return x.transpose(1, 2).contiguous().view(batch_size, sequence_size, num_heads * head_size)

    def forward(self, Q, K, V, mask=None):
        # mask Shape: (batch_size, 1, sequence_size, sequence_size)
        # Q    Shape: (batch_size,    sequence_size, feature_size )
        Q    = self.split_heads(self.W_q_(self.W_q(Q)))  # Shape: (batch_size, num_heads, sequence_size, head_size )
        K    = self.split_heads(self.W_k_(self.W_k(K)))  # Shape: (batch_size, num_heads, sequence_size, head_size )
        V    = self.split_heads(self.W_v_(self.W_v(V)))  # Shape: (batch_size, num_heads, sequence_size, head_size )
        attn_output = self.scaled_dot_product_attention(Q, K, V, mask)
        output      = self.W_o_(self.W_o(self.combine_heads(attn_output)))
