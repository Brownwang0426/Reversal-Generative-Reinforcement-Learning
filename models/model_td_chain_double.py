...
class build_model(nn.Module):
    def __init__(self,
        ...
        self.positional_encoding  = nn.Parameter(self.generate_positional_encoding((self.history_size + 1 + self.future_size) * 2, self.feature_size ), requires_grad=False)
        ...
        seq_len                   = (self.history_size + 1 + self.future_size) * 2
        group_idx                 = torch.arange(seq_len) // 2
    ...
    def forward_(self, history_r, history_s, history_a, present_r, present_s, present_a, future_r, future_s, future_a):
        ...
        num_groups = 1 + future_s.size(1)
        h = h[:, -(num_groups * 2):, :]
        s = self.state_linear_ (h[:, 0::2, :])
        r = self.reward_linear_(h[:, 1::2, :])
        r = torch.tanh(r)
